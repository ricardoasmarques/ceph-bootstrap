...
def purge(non_interactive):
    """
    Destroy all ceph clusters
    """
    _prompt_proceed('You are about to permanently remove ceph cluster(s). Proceed?', 'n')
    _prompt_proceed('Proceed, even though this may destroy valuable data?', 'n')
    retcode = run_purge(non_interactive)
    sys.exit(retcode)
